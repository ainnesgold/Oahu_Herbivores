...
# fishing_effort_grid <- fishing_effort_grid[1]
...
for (iter in 1:nrow(parameter_grid)) {
  ...
  for(t in 2:timesteps){#timesteps){
    ...
    # Need to check after dispersal of both adults and recruits
    # If one patch is above carrying capacity, then force population to carrying capacity for that patch and make the other fish "spillover" to the patch not at carrying capacity
    # Else if both are above carrying capacity, then set both to the carrying capacity - essentially forcing density dependence
